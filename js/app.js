...
function getEmergencyNumber() {
  // Try to detect timezone/language locale as a proxy
...
function generateRecommendation() {
  ...
  if (isEmergency) {
    ...
    return {
      ...
      icon: 'siren',
      color: '#EF4444',
      summary: `Based on your reported symptom "${emergencyReason}", this requires immediate emergency medical attention. Do not wait.`,
      actions: [
        { type: 'immediate', text: 'Call emergency services RIGHT NOW', icon: 'phone-call' },
      ...
      redFlags: selectedCondition.redFlags || [],
      firstAidSteps: selectedCondition.firstAidSteps || [],
      followUp: 'Emergency services will guide you.',
  ...
  // Override with condition's base severity
  ...
  const severityConfig = {
    low: { title: 'Self-Care Recommended', icon: 'check-circle', color: '#10B981', gradient: 'linear-gradient(135deg, #ECFDF5, #D1FAE5)' },
    moderate: { title: 'Medical Consultation Advised', icon: 'stethoscope', color: '#F59E0B', gradient: 'linear-gradient(135deg, #FFFBEB, #FEF3C7)' },
    high: { title: 'Urgent Medical Attention Needed', icon: 'alert-circle', color: '#EF4444', gradient: 'linear-gradient(135deg, #FEF2F2, #FEE2E2)' },
    emergency: { title: 'Emergency — Seek Immediate Help', icon: 'siren', color: '#7F1D1D', gradient: 'linear-gradient(135deg, #FEF2F2, #FCA5A5)' }
  };

  const cfg = severityConfig[severityLevel];
  const template = selectedCondition.recommendationTemplate?.[severityLevel] || '';

  const actions = [];
  if (selectedCondition.firstAidSteps && selectedCondition.firstAidSteps.length > 0) {
    actions.push({ type: 'immediate', text: selectedCondition.firstAidSteps[0], icon: 'zap' });
  }
  actions.push({ type: 'monitor', text: `Monitor: ${(selectedCondition.redFlags || []).slice(0, 2).join(', ') || 'worsening symptoms'}`, icon: 'activity' });
  if (severityLevel !== 'low') {
    actions.push({ type: 'whenToSeeDoctor', text: severityLevel === 'high' ? 'See a doctor within 24 hours' : 'Schedule a doctor\'s appointment this week', icon: 'calendar' });
  }
  actions.push({ type: 'medication', text: 'Follow only prescribed or recommended OTC medications. Do not self-medicate beyond guidelines.', icon: 'pill', warning: 'Always read dosage instructions' });
  ...
  return {
    ...
    redFlags: selectedCondition.redFlags || [],
    firstAidSteps: selectedCondition.firstAidSteps || [],
    followUp: selectedCondition.severity === 'emergency' ? 'Seek emergency care if symptoms escalate.' : `If symptoms don't improve in ${severityLevel === 'low' ? '5–7 days' : '24–48 hours'}, consult a healthcare provider.`,
    disclaimer: t('disclaimer'),
    didYouKnow: selectedCondition.didYouKnow || '',
    prevention: selectedCondition.prevention || [],
    conditionName: selectedCondition.name,
    specialtyName: window.HoloCareState.selectedSpecialty?.name || ''
  };
}
...
function renderSpecialties(filter = '') {
  ...
  const filtered = filter
    ? specialties.filter(s => s.name.toLowerCase().includes(filter.toLowerCase()) || s.description.toLowerCase().includes(filter.toLowerCase()))
    : specialties;
  ...
  container.innerHTML = sorted.map(spec => {
    ...
    return `
      <div class="specialty-card ${spec.isEmergency ? 'specialty-card--emergency' : ''}"
           ...
           role="button"
           tabindex="0"
           aria-label="${spec.name}: ${spec.description}, ${count} conditions"
        ...
        <div class="specialty-card__content">
          <h3 class="specialty-card__name">${spec.name}</h3>
          <p class="specialty-card__desc">${spec.description}</p>
        </div>
...
function renderConditions() {
  ...
  if (subtitle) subtitle.textContent = spec?.name || '';
  ...
  const severityConfig = {
    low: { label: 'Self-Care', color: '#10B981', bg: '#ECFDF5' },
  ...
  container.innerHTML = conditions.map(cond => {
    ...
    return `
      <div class="condition-card ${cond.severity === 'emergency' ? 'condition-card--emergency' : ''}"
           ...
           role="button"
           tabindex="0"
           aria-label="${cond.name}: ${cond.description}. Severity: ${sev.label}"
        ...
        <h3 class="condition-card__name">${cond.name}</h3>
        <p class="condition-card__desc">${cond.description}</p>
...
function renderQuestion(index) {
  ...
  if (condNameEl) condNameEl.textContent = condition.name;
    ...
    inputHTML = `<div class="options-grid" role="radiogroup" aria-labelledby="question-text">
      ${(q.options || []).map((opt, i) => `
        <label class="option-card ${savedAnswer === opt ? 'option-card--selected' : ''}" 
          ...
          <span class="option-card__text">${opt}</span>
        </label>
      `).join('')}
    </div>`;
  } else if (q.type === 'multiple') {
    ...
    inputHTML = `<div class="options-pills" role="group" aria-labelledby="question-text">
      ${(q.options || []).map(opt => `
        <label class="option-pill ${saved.includes(opt) ? 'option-pill--selected' : ''}" 
          ...
          <i data-lucide="check" aria-hidden="true"></i> ${opt}
        </label>
      `).join('')}
    </div>
  ...
  container.innerHTML = `
    <div class="question-card" role="group">
      ...
      <h2 class="question-text" id="question-text">${q.question}</h2>
  ...
  if (nextBtn) {
    nextBtn.textContent = index === total - 1 ? 'Submit' : t('next');
    nextBtn.onclick = () => {
...
function showAnalyzing() {
  ...
  container.innerHTML = `
    <div class="analyzing-screen" role="status" aria-live="polite">
      ...
      <p>Generating personalized health guidance</p>
...
function renderRecommendation(rec) {
  ...
  const actionLabels = { immediate: 'Immediate Action', medication: 'Medication', monitor: 'Monitor', whenToSeeDoctor: 'See a Doctor' };
  ...
  container.innerHTML = `
    <div class="rec-hero" style="background: ${rec.gradient || 'linear-gradient(135deg,#ECFDF5,#D1FAE5)'}">
      ...
      <div class="rec-hero__text">
        ...
        <p class="rec-condition">Based on: <strong>${rec.conditionName}</strong>${rec.specialtyName ? ' · ' + rec.specialtyName : ''}</p>
      </div>
    </div>

    <div class="rec-summary glass-card">
      <h3><i data-lucide="file-text" aria-hidden="true"></i> Assessment Summary</h3>
      <p id="rec-summary-text" class="rec-summary-text">${rec.summary}</p>
    </div>

    <div class="rec-actions">
      <h3 class="rec-section-title">Recommended Actions</h3>
...
function showEmergencyProtocol(reason) {
  ...
  const firstAidSteps = condition?.firstAidSteps || [];
  ...
  overlay.innerHTML = `
    <div class="emergency-content" role="alertdialog" aria-modal="true" aria-labelledby="em-title" aria-describedby="em-desc">
      ...
      <h1 id="em-title" class="em-title">EMERGENCY</h1>
  ...
  overlay.addEventListener('keydown', function trapFocus(e) {
    if (e.key === 'Escape') return; // Don't allow escape
    if (e.key === 'Tab') {
...
function applyTranslations() {
  ...
  // Re-render dynamic sections if they are currently visible
  ...
  // Re-init lucide icons in case new DOM was injected
  if (window.lucide) lucide.createIcons();
}
...
function downloadReport() {
  ...
  const condition = window.HoloCareState.selectedCondition;
  if (!rec || !condition) { showToast('No report available', 'warning'); return; }

  const now = new Date().toLocaleDateString('en-GB', { year: 'numeric', month: 'long', day: 'numeric' });
  const printWin = window.open('', '_blank');
  printWin.document.write(`
    <!DOCTYPE html><html><head>
    <title>HoloCare Health Report — ${condition.name}</title>
    <style>
    ...
    <p><strong>Specialty:</strong> ${window.HoloCareState.selectedSpecialty?.name || 'N/A'}</p>
...
function init() {
  ...
  // If not on hero, render hero and go to hero
  ...
  // PART 1 INTEGRATION — Save progress to localStorage
